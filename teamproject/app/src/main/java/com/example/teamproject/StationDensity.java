...
class ResearchRecord{
    ...
    public HashMap getTimeIndex(){
        return time_index;
    }

    String name;
    HashMap<String, HashMap<String, ArrayList<Float>>> time_index = new HashMap<>();
//    HashMap<Integer, HashMap<String, ArrayList<Float>>> last_index = new HashMap<>();
    ...
    public ResearchRecord(String name){
        ...
        time_index.put(String.valueOf(st[index].getLine()[0][0]), t1);
//        t1.put("uptime", f);
//        t1.put("downtime", f);
        if(st[index].getLine()[1][0] != -1){
            time_index.put(String.valueOf(st[index].getLine()[1][0]), t1);
        }
        t1.put("uptime", f);
        t1.put("downtime", f);
//        HashMap<String, ArrayList<Float>> t1 = time_index.get(String.valueOf(s.getStationLine()[index][0]));
        ArrayList<Float> t3 = time_index.get(String.valueOf(s.getStationLine()[index][0])).get("uptime");
    }
    public void addDensity(int line, int ud, int index, float f){
        float nf;
        System.out.println("\n이름"+ name +"\n");
        for(String l : time_index.keySet()){
            System.out.println("line: "+l);

        }
        HashMap<String, ArrayList<Float>> t = time_index.get(String.valueOf(line));
        for(String s : t.keySet()){
            System.out.println("key: "+ s);
        }
        if (ud == 0) {
//            ArrayList<Float> t1 = time_index.get(String.valueOf(line)).get("uptime");
            nf = time_index.get(String.valueOf(line)).get("uptime").get(index) + f;
            time_index.get(String.valueOf(line)).get("uptime").set(index, nf);
        } else {
            nf = time_index.get(String.valueOf(line)).get("downtime").get(index) + f;
            time_index.get(String.valueOf(line)).get("downtime").set(index, nf);
        }
    }
        ...
        database_ref.child("station").get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
            ...
            public void onComplete(@NonNull Task<DataSnapshot> task) {
                ...
                for(int i = a.getPath_cnt() - 1; i >= 0; i--){
                    ...
                    if(st_list[a.getPath()[i]].equals(a.getTrans()[n])) {
...
//                    System.out.println("b: "+s.getStationList()[b.getPath()[i]]);
                    ...
                    if(st_list[b.getPath()[i]].equals(b.getTrans()[n])) {
                        n++;
                        if(n == b.getSum_t()){
                            break;
                        }
...
//                    System.out.println("c: "+s.getStationList()[c.getPath()[i]]);
                    ...
                    if(st_list[c.getPath()[i]].equals(c.getTrans()[n])) {
                        n++;
                        if(n == c.getSum_t()){
                            break;
                        }
                ...
                for(int i = 0; i < 111; i++){
                    if(path[i] == 1){
                        ...
                        if(r == null){
                            ...
                            System.out.println("   call   "+st_list[i]);
